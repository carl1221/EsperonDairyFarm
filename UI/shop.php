  ...
  grid.innerHTML = list.map(p => {
    ...
    return `<div class="product-card${outOfStock ? ' out-of-stock' : ''}">
      ...
      <div class="product-card__footer">
        ...
        <button class="btn-add" onclick="addToCart(${p.product_id})" ${outOfStock ? 'disabled' : ''}>
          <span class="material-symbols-outlined" style="font-size:0.9rem;">add_shopping_cart</span>
          Add
        </button>
      </div>
    </div>`;
